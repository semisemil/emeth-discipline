...
function projectFor(cwd) {
  ...
  for (let depth = 0; depth < 12; depth += 1) {
    if (fs.existsSync(path.join(directory, '.proofline', 'architecture.json'))) migrateProject(directory);
...
function notice(input, options = {}) {
  ...
  const data = options.dataRoot || process.env.PLUGIN_DATA || path.join(os.tmpdir(), 'emeth-plugin-state');
  ...
  return `While .emeth/architecture.json exists and memory at ${JSON.stringify(linked.root)} is managed, read ${JSON.stringify(skill)} for architecture-dependent work or new durable project context. Reuse unchanged evidence; mechanical edits need no lookup.`;
}
